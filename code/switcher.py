...
# The list of windows matching the name provided by the user.
...
class Actions:
    ...
    def GetAllWindowsMatchingName(name: str) -> List[ui.Window]:
        ...
        # the given name with a name from the list. This is a form of fuzzy
        # match to help match some hard to dictate apps.
        # If the given name is particularly short (< 3 characters) treat
        # it as if no matches were found.
        if not matchingWindows and len(name) >= 3:
            for running_name, full_application_name in ctx.lists["self.running"].items():
                if running_name == name or running_name.lower().startswith(name.lower()):
                    name = full_application_name
                    break

        # Try to match the window again, but this time after the fuzzy match.
    ...
    def AllMatchingWindowsClose():
        """Hides the UI showing all matching windows"""
    ...
    def FocusMultiMatchWindowByNumber(index: int):
        """Selects a window based on the current cached index into MultiMatchWindowList"""
        global MultiMatchWindowList

        actualIndex = index-1 # UI displays 1 indexed list

        if actualIndex >= 0 and actualIndex < len(MultiMatchWindowList):
            actions.user.switcher_focus_window(MultiMatchWindowList[actualIndex])
...
@imgui.open()
def ShowMatchingWindowUI(gui: imgui.GUI):
    global MultiMatchWindowList

    gui.text("Matching Windows (Say: 'choose (# | close)')")
    gui.line()

    for index, window in enumerate(MultiMatchWindowList):
        if gui.button("{}. {} -- {}".format(index+1, window.title, window.app.name)):
